...
@Service
public class GetCategoryCommodityListServiceImpl implements GetCategoryCommodityListService {
    ...
    public List<Map<String,Object>> GetCategoryCommodityList() {
        ...
        sellerQueryWrapper.eq("open_id",user.getOpenId());

        Seller seller = sellerMapper.selectOne(sellerQueryWrapper);
        if(seller == null) {
            throw new RuntimeException("您没有创建商家");
        }
